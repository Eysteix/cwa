...
export function Scenarios({ scenarios, mode = 'cwa' }: Props) {
  ...
  const label = isGpa ? 'Avg needed to boost CGPA by...' : 'Avg needed to boost CWA by...'
  return (
    <div>
      <div style={{ fontFamily: 'var(--font-mono)', fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.07em', color: 'var(--ink-3)', marginBottom: 8 }}>{label}</div>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8 }}>
        {scenarios.map(({ pct, requiredAvg }) => (
          <div key={pct} style={{ background: 'var(--bg)', borderRadius: 'var(--r)', padding: '12px 6px', textAlign: 'center' }}>
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: 'var(--ink-3)', textTransform: 'uppercase' }}>+{pct}%</div>
            ...
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: 'var(--ink-3)' }}>avg needed</div>
          </div>
        ))}
      </div>
    </div>
