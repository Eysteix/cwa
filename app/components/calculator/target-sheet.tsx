...
export function TargetSheet({ open, onClose, status, semesterCredits, quickTargets }: Props) {
  ...
  return (
    <>
      ...
      <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, background: 'var(--surface)', borderRadius: '16px 16px 0 0', padding: 24, zIndex: 201, maxWidth: 500, margin: '0 auto', boxShadow: 'var(--sh-lg)' }}>
        ...
        <div style={{ fontFamily: 'var(--font-heading)', fontSize: 16, fontWeight: 800, marginBottom: 16 }}>Target CWA Calculator</div>
        <div style={{ marginBottom: 14 }}>
          <label style={{ fontFamily: 'var(--font-mono)', fontSize: 9, fontWeight: 600, textTransform: 'uppercase' as const, letterSpacing: '0.07em', color: 'var(--ink-3)', display: 'block', marginBottom: 4 }}>Enter Target CWA</label>
          <input type="number" value={target} onChange={e => setTarget(e.target.value)} placeholder="e.g. 80" style={inputStyle} />
        </div>
        {required !== null && (
          <div style={{ background: 'var(--orange-dim)', border: '1px solid var(--orange-border)', borderRadius: 4, padding: '12px 14px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
            ...
            <span style={{ fontFamily: 'var(--font-heading)', fontSize: 28, fontWeight: 800, color: required > 100 ? '#ef4444' : 'var(--orange)' }}>
              {required > 100 ? '>100' : required < 0 ? 'N/A' : required.toFixed(2)}
            </span>
          </div>
        )}
        <div>
