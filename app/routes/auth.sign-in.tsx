...
export function meta(_: Route.MetaArgs) {
  return [{ title: 'Sign in — FirstTarget' }]
}
...
export default function SignInPage() {
  ...
  const panelStyle: React.CSSProperties = { background: '#27272a', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 48, position: 'relative', overflow: 'hidden' }
  ...
  return (
    <div className="auth-grid" style={{ minHeight: '100vh' }}>
      <div className="auth-left-panel" style={panelStyle}>
        ...
        <Link to="/" style={{ position: 'absolute', top: 28, left: 32, textDecoration: 'none', display: 'flex', alignItems: 'center', gap: 7, zIndex: 2 }}>
          <span style={{ fontFamily: 'var(--font-heading)', fontSize: 17, fontWeight: 800, color: 'white', letterSpacing: '-0.4px' }}>First</span>
          <span style={{ fontFamily: 'var(--font-heading)', fontSize: 17, fontWeight: 800, color: 'var(--orange)', letterSpacing: '-0.4px' }}>Target</span>
        </Link>
        ...
        <div style={{ position: 'relative', zIndex: 1, textAlign: 'center', marginTop: 32 }}>
          <h2 style={{ fontFamily: 'var(--font-heading)', fontSize: 22, fontWeight: 800, color: 'white', lineHeight: 1.25 }}>
            {auth.heading.split('academic success').map((part, i) =>
              i === 0 ? <span key={i}>{part}<span style={{ color: 'var(--orange)' }}>academic success</span></span> : <span key={i}>{part}</span>
            )}
          </h2>
      ...
      <div className="auth-right-panel" style={{ background: 'var(--surface)', display: 'flex', flexDirection: 'column', padding: '0 56px', justifyContent: 'center', position: 'relative' }}>
        ...
        <div style={{ width: '100%' }}>
          <h1 style={{ fontFamily: 'var(--font-heading)', fontSize: 24, fontWeight: 800, color: 'var(--ink)', marginBottom: 4 }}>Welcome back</h1>
          <p style={{ fontFamily: 'var(--font-body)', fontSize: 13, color: 'var(--ink-3)', marginBottom: 28 }}>Sign in to access your saved progress</p>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div style={{ marginBottom: 6 }}><label style={labelStyle}>Password</label><input type="password" value={password} onChange={e => setPassword(e.target.value)} placeholder="••••••••" required style={inputStyle} /></div>
            {error && <p style={{ fontFamily: 'var(--font-body)', fontSize: 12, color: '#ef4444', marginBottom: 8 }}>{error}</p>}
            <button type="submit" disabled={loading} style={{ fontFamily: 'var(--font-heading)', width: '100%', padding: 13, background: 'var(--ink)', color: 'white', border: 'none', borderRadius: 6, fontSize: 14, fontWeight: 700, cursor: loading ? 'not-allowed' : 'pointer', opacity: loading ? 0.7 : 1, marginTop: 6 }}>{loading ? 'Signing in…' : 'Sign in'}</button>
          </form>
          <p style={{ fontFamily: 'var(--font-body)', marginTop: 20, textAlign: 'center', fontSize: 13, color: 'var(--ink-3)' }}>
            No account yet? <Link to="/auth/sign-up" style={{ color: 'var(--orange)', fontWeight: 700, textDecoration: 'none' }}>Create one free</Link>
          </p>
        </div>
      </div>
    </div>
