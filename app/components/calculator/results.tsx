...
export function Results({ results, mode = 'cwa' }: Props) {
  ...
  const avgLabel = isGpa ? 'New GPA' : 'New CWA'
  const semLabel = isGpa ? 'Sem GPA' : 'Sem Average'
  const marksLabel = isGpa ? 'Grade Points' : 'New Marks'
  const marksVal = isGpa ? newTotalMarks.toFixed(2) : newTotalMarks.toLocaleString()
  return (
    <div style={{ marginBottom: 12 }}>
      <div style={{ fontFamily: 'var(--font-mono)', fontSize: 9, textTransform: 'uppercase', letterSpacing: '0.08em', color: 'var(--ink-3)', marginBottom: 10 }}>Step 3 — Projected Results</div>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
        <div style={tileStyle}><div style={valStyle(true)}>{newCWA.toFixed(2)}</div><div style={lblStyle}>{avgLabel} {sign}{cwaDelta.toFixed(2)}</div></div>
